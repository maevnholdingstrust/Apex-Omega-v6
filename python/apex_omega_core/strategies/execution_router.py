...
class ExecutionRouter:
            ...
            return {
                ...
                'c2': {
                    ...
                    'execution': {
                        ...
                        'executed': False,
                        'blocked': True,
                        'reason': 'C1_FORK_SIM_FAILED',
                    },
                },
                'state_reload': {
                    'post_c1_route': copy.deepcopy(route),
                },
                'eip1559_params': eip1559_params,
                'gas_cost_usd': effective_gas_cost,
                'pipeline_gate': {
                    'p_fill': p_fill,
                    'c1_should_execute': False,
                    'c2_should_execute': False,
    ...
    def _fork_sim_passed(fork_sim: dict) -> bool:
        ...
        if not isinstance(fork_sim, dict):
            return False
        return str(fork_sim.get('status', '')).upper() == 'PASS'

    @staticmethod
    def _blocked_execution(leg: str, fork_sim: dict, decision: str = 'EXECUTE') -> dict:
        reason = f'{leg}_FORK_SIM_FAILED'
        if isinstance(fork_sim, dict) and fork_sim.get('reason'):
            reason = f"{reason}: {fork_sim.get('reason')}"
        return {
            'tx_hash': None,
            'decision': decision,
            'executed': False,
