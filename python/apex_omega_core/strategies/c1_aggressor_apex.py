...
class C1AggressorApex:
    ...
    async def execute_contract_strike(self, strike_plan: dict) -> ExecutionResult:
        ...
        if strike_plan.get('action') != 'STRIKE':
            return ExecutionResult(success=False, revert_reason="strike plan action is not STRIKE")
        ...
        accepted = bool(tx_hash) and (
            invocation.get('executed_onchain') or broadcast.get('status') == "submitted" or broadcast.get('status') == 1
        )
