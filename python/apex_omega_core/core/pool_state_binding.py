...
from .v3_tick_fetcher import fetch_initialized_ticks_around
from .venue_math_contracts import assert_execution_grade
...
def bind_v2_pool_state(pool: str, token_in_symbol: str, token_out_symbol: str, fee_bps: float) -> BoundPoolState:
    w3 = get_w3(); pair = w3.eth.contract(address=Web3.to_checksum_address(pool), abi=V2_PAIR_ABI)
    r0, r1, _ = pair.functions.getReserves().call(); token0 = pair.functions.token0().call(); token1 = pair.functions.token1().call()
    token_in = TOKENS[token_in_symbol].address.lower(); token_out = TOKENS[token_out_symbol].address.lower()
    if token0.lower() == token_in and token1.lower() == token_out:
        reserve_in = r0 / (10 ** TOKENS[token_in_symbol].decimals); reserve_out = r1 / (10 ** TOKENS[token_out_symbol].decimals)
    elif token1.lower() == token_in and token0.lower() == token_out:
        reserve_in = r1 / (10 ** TOKENS[token_in_symbol].decimals); reserve_out = r0 / (10 ** TOKENS[token_out_symbol].decimals)
    else:
...
def bind_v3_pool_state(pool: str, token_in_symbol: str, token_out_symbol: str, fee: int, tick_spacing: int = 60, word_radius: int = 8) -> BoundPoolState:
    w3 = get_w3(); p = w3.eth.contract(address=Web3.to_checksum_address(pool), abi=V3_POOL_ABI)
    slot0 = p.functions.slot0().call(); liquidity = p.functions.liquidity().call(); token0 = p.functions.token0().call(); token1 = p.functions.token1().call()
    tick_fetch = fetch_initialized_ticks_around(pool, int(slot0[1]), tick_spacing, word_radius=word_radius)
    state = {
        "sqrt_price_x96": slot0[0],
        "tick": slot0[1],
        "liquidity": liquidity,
        "fee": fee,
        "tick_bitmap_or_tick_lens": tick_fetch.initialized_ticks,
        "token0": token0,
        "token1": token1,
    ...
    try:
        ...
        return BoundPoolState("v3", pool, token_in_symbol, token_out_symbol, state, True, "v3 initialized ticks bound")
    except Exception as exc:
