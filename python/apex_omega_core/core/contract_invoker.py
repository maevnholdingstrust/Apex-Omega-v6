...
from .execution_transport_selector import select_execution_transport
...
class ContractInvoker:
    ...
    def _event_base(
        ...
        *,
        ...
        return {
            ...
            "strike_role": ctx.get("strike_role"),
            "c2_decision": ctx.get("c2_decision"),
            "idempotency_key": idempotency_key,
            "chain_id": chain_id,
            "chain_name": chain_name_for(chain_id),
            "executor_contract": self.target_address,
            "wallet_address": ctx.get("wallet_address") or getattr(self.account, "address", None),
            ...
            "realized_profit_usd": ctx.get("realized_profit_usd"),
    ...
    def _record_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stored = state_store.append(event) if state_store is not None else event
        # Dispatch notification via the bounded pool — never blocks the trading path.
        telegram = getattr(self, "_telegram", None)
        if telegram is not None:
            _NOTIFY_POOL.submit(telegram.send_event, stored)
        return stored

    def _send_signed_transaction(self, raw_transaction: bytes, chain_id: int) -> tuple[Any, str]:
        """Submit signed bytes through the chain-specific write lane only."""
        transport = select_execution_transport(chain_id=chain_id)
        if chain_id == 137 and transport.mode != "polygon_private_mempool":
            raise RuntimeError(
                "Polygon private submission is not configured. "
                "Set POLYGON_PRIVATE_MEMPOOL_RPC_URL to the provisioned "
                "Polygon Private Mempool submission-only endpoint. "
                "Builder relays such as Titan require the bundle submitter, "
                "not raw eth_sendRawTransaction."
            )
        if not transport.url:
            raise RuntimeError(f"No transaction submission transport configured for chain_id={chain_id}")
        submission_w3 = Web3(Web3.HTTPProvider(transport.url, request_kwargs={"timeout": 10}))
        return submission_w3.eth.send_raw_transaction(raw_transaction), transport.mode
    ...
    def invoke(
        ...
        if getattr(self.w3, "is_connected", lambda: False)():
        ...
        try:
            ...
            tx_hash_bytes, submission_mode = self._send_signed_transaction(raw_transaction, int(chain_id))
            tx_hash = Web3.to_hex(tx_hash_bytes)
            result["tx_hash"] = tx_hash
            result["submission_transport"] = submission_mode
        except Exception as exc:
        ...
        submitted_event = self._record_event(
            {
                **base_event,
                "status": "submitted",
                "tx_hash": tx_hash,
                "submission_transport": submission_mode,
