...
def _fetch_v2_quote(venue_name: str, base: str, quote: str) -> MarketQuote | None:
    ...
    if not venue.factory:
        return None
    w3 = get_w3()
    factory = w3.eth.contract(address=_checksum(venue.factory), abi=V2_FACTORY_ABI)
    pair_addr = factory.functions.getPair(_checksum(base), _checksum(quote)).call()
    if not pair_addr or pair_addr.lower() == ZERO:
        return None
    pair = w3.eth.contract(address=_checksum(pair_addr), abi=V2_PAIR_ABI)
    r0, r1, _ = pair.functions.getReserves().call()
    token0 = pair.functions.token0().call()
    token1 = pair.functions.token1().call()
    dec0, dec1 = _decimals(token0), _decimals(token1)
    h0, h1 = r0 / (10 ** dec0), r1 / (10 ** dec1)
    if h0 <= 0 or h1 <= 0:
        return None
    token0_symbol, token1_symbol = _symbol(token0), _symbol(token1)
    # price quote per base
    if token0.lower() == base.lower() and token1.lower() == quote.lower():
        price = h1 / h0
        liquidity_hint = (h0 * price) + h1
    elif token1.lower() == base.lower() and token0.lower() == quote.lower():
        price = h0 / h1
        liquidity_hint = (h1 * price) + h0
    else:
        return None
...
def _fetch_v3_quote(venue_name: str, base: str, quote: str) -> MarketQuote | None:
    ...
    if not venue.factory:
        return None
    w3 = get_w3()
    factory = w3.eth.contract(address=_checksum(venue.factory), abi=V3_FACTORY_ABI)
    best: MarketQuote | None = None
    for fee in V3_FEES:
        pool_addr = factory.functions.getPool(_checksum(base), _checksum(quote), fee).call()
        if not pool_addr or pool_addr.lower() == ZERO:
            continue
        pool = w3.eth.contract(address=_checksum(pool_addr), abi=V3_POOL_ABI)
        ...
        dec0, dec1 = _decimals(token0), _decimals(token1)
        token1_per_token0 = _v3_price_token1_per_token0(slot0[0], dec0, dec1)
        if token0.lower() == base.lower() and token1.lower() == quote.lower():
            price = token1_per_token0
        elif token1.lower() == base.lower() and token0.lower() == quote.lower():
            price = 1 / token1_per_token0 if token1_per_token0 else 0
        else:
            continue
        quote_obj = MarketQuote(venue_name, pool_addr, "v3", fee / 100, _symbol(base), _symbol(quote), price, float(liquidity))
        if best is None or quote_obj.liquidity_hint > best.liquidity_hint:
            best = quote_obj
    return best
...
def scan_multi_market(max_pairs: int = 24, min_spread_bps: float = 40.0) -> list[ScannerOpportunity]:
    ...
    token_pairs = list(combinations(symbols, 2))[:max_pairs]
    ...
    for base_symbol, quote_symbol in token_pairs:
        ...
        for venue_name, venue in SUPPORTED_EXECUTION_VENUES.items():
            try:
                ...
                if q and q.price_quote_per_base > 0:
                    quotes.append(q)
            except Exception:
                continue
        if len(quotes) < 2:
            continue
        buy = min(quotes, key=lambda x: x.price_quote_per_base)
        sell = max(quotes, key=lambda x: x.price_quote_per_base)
        if buy.pool == sell.pool or sell.price_quote_per_base <= buy.price_quote_per_base:
            continue
