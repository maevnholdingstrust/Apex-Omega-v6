...
def _valid_audit_kwargs(**overrides):
    a_in = 1000.0; b_out_1 = 995.0; a_out_2 = 1008.5
    p_gross = a_out_2 - a_in; c_total_exec = 1.0; p_net = p_gross - c_total_exec
    base = dict(a_in=a_in, fee1=0.003, b_out_1=b_out_1, b_in_2=b_out_1, fee2=0.0025, a_out_2=a_out_2, p_gross=p_gross, p_net=p_net, c_total_exec=c_total_exec)
...
class TestAuditTwoLegRouteEnvelope:
    ...
    def test_tolerance_boundary(self):
        kwargs = _valid_audit_kwargs(); b = kwargs["b_out_1"]; kwargs["b_in_2"] = b + 1e-9
        assert audit_two_leg_route_envelope(**kwargs).passed is True
    def test_just_above_tolerance_triggers_violation(self):
        kwargs = _valid_audit_kwargs(); b = kwargs["b_out_1"]; kwargs["b_in_2"] = b + 1e-7
...
class TestExecutionDegradationSimulator:
    def _make_sim(self, seed=42, mean=0.65, std=0.35):
        return ExecutionDegradationSimulator(degradation_mean=mean, degradation_std=std, rng=random.Random(seed))
    def _base_kwargs(self, c2_decision="STRIKE"):
        return dict(a_in=1000.0, b_out_1=995.0, a_out_2=1008.5, p_gross=8.5, p_net_deterministic=7.5, c_total_exec=1.0, p_fill=0.9, c2_decision=c2_decision, fee1=0.003, fee2=0.0025)
    ...
    def test_do_nothing_audit_passes(self):
        assert self._make_sim().simulate_one_run(**self._base_kwargs(c2_decision="DO_NOTHING")).audit.passed is True
    def test_deterministic_with_same_seed(self):
        kwargs = self._base_kwargs()
        assert self._make_sim(seed=7).simulate_one_run(**kwargs).p_net_actual == pytest.approx(self._make_sim(seed=7).simulate_one_run(**kwargs).p_net_actual)
...
class TestBatchSimulator:
    ...
    POOL_KWARGS = dict(a_in=5000.0, fee1=0.003, r1_in=2_000_000.0, r1_out=2_100_000.0, fee2=0.0025, r2_in=2_100_000.0, r2_out=2_050_000.0, c_total_exec=0.5, p_fill=0.9)
    ...
    def test_total_profit_equals_sum_of_mean_times_runs(self):
        summary = self._make_batch_sim().run(**self.POOL_KWARGS, n_runs=100)
        assert summary.total_actual_profit == pytest.approx(summary.mean_actual_profit_per_run * summary.n_runs, rel=1e-9)
    def test_ev_equals_p_net_times_p_fill(self):
        math = SlippageSentinel().two_leg_arb_profit(a_in=5000.0, fee1=0.003, r1_in=2_000_000.0, r1_out=2_100_000.0, fee2=0.0025, r2_in=2_100_000.0, r2_out=2_050_000.0, c_gas=0.5)
    ...
    def test_do_nothing_when_p_fill_zero(self):
        kwargs = dict(self.POOL_KWARGS); kwargs["p_fill"] = 0.0
        summary = BatchSimulator(SlippageSentinel(), ExecutionDegradationSimulator(rng=random.Random(1))).run(**kwargs, n_runs=20)
...
class TestSSOTPipelineFinalizer:
    POOL_STATE = dict(fee1=0.003, r1_in=2_000_000.0, r1_out=2_100_000.0, fee2=0.0025, r2_in=2_100_000.0, r2_out=2_050_000.0, c_total_exec=0.5)
    SIZES = [500.0, 1000.0, 2000.0, 3000.0, 5000.0, 8000.0, 12000.0]
    def _make_finalizer(self, seed=42, p_fill=0.9, n_batch=50):
        return SSOTPipelineFinalizer(sizes_to_test=self.SIZES, n_batch_runs=n_batch, p_fill=p_fill, rng_seed=seed)
    ...
    def test_interior_optimum_not_always_largest_size(self):
        shallow = dict(fee1=0.003, r1_in=50_000.0, r1_out=52_500.0, fee2=0.0025, r2_in=52_500.0, r2_out=51_000.0, c_total_exec=0.1)
        sizes = [100.0, 500.0, 1000.0, 2000.0, 5000.0, 10000.0, 20000.0]
        result = SSOTPipelineFinalizer(sizes_to_test=sizes, n_batch_runs=10, p_fill=0.9, rng_seed=0).run(**shallow)
    ...
    def test_c2_do_nothing_when_p_fill_zero(self):
        result = SSOTPipelineFinalizer(sizes_to_test=self.SIZES, n_batch_runs=10, p_fill=0.0, rng_seed=0).run(**self.POOL_STATE)
        assert result.c2_decision == "DO_NOTHING"
    def test_ev_equals_p_net_times_p_fill(self):
        p_fill = 0.75
        result = SSOTPipelineFinalizer(sizes_to_test=self.SIZES, n_batch_runs=10, p_fill=p_fill, rng_seed=0).run(**self.POOL_STATE)
    ...
    def test_total_profit_consistent_with_mean(self):
        ...
        assert bs.total_actual_profit == pytest.approx(bs.mean_actual_profit_per_run * bs.n_runs, rel=1e-9)
    def test_no_spread_produces_do_nothing(self):
        no_spread = dict(fee1=0.003, r1_in=1_000_000.0, r1_out=1_000_000.0, fee2=0.003, r2_in=1_000_000.0, r2_out=1_000_000.0, c_total_exec=0.01)
...
class TestSSOTPipelineWithLiveData:
    ...
    def _make_finalizer(self, p_fill: float = 0.9, n_batch: int = 30) -> SSOTPipelineFinalizer:
        return SSOTPipelineFinalizer(sizes_to_test=self.SIZES, n_batch_runs=n_batch, p_fill=p_fill, rng_seed=42)
    def test_pipeline_returns_final_result(self, live_pool_state):
        assert isinstance(self._make_finalizer().run_live_state(live_pool_state), PipelineFinalResult)
    def test_best_size_is_from_candidates(self, live_pool_state):
        assert self._make_finalizer().run_live_state(live_pool_state).best_size in self.SIZES
    def test_audit_passes_for_live_plan(self, live_pool_state):
        result = self._make_finalizer().run_live_state(live_pool_state)
    ...
    def test_c2_decision_is_valid_string(self, live_pool_state):
        assert self._make_finalizer().run_live_state(live_pool_state).c2_decision in {"STRIKE", "DO_NOTHING"}
    def test_ev_equals_p_net_times_p_fill(self, live_pool_state):
        p_fill = 0.8
        result = self._make_finalizer(p_fill=p_fill).run_live_state(live_pool_state)
    ...
    def test_batch_summary_hit_rate_in_range(self, live_pool_state):
        result = self._make_finalizer(n_batch=50).run_live_state(live_pool_state)
        assert 0.0 <= result.batch_summary.hit_rate <= 1.0
    def test_total_profit_consistent_with_mean(self, live_pool_state):
        bs = self._make_finalizer(n_batch=50).run_live_state(live_pool_state).batch_summary
        assert bs.total_actual_profit == pytest.approx(bs.mean_actual_profit_per_run * bs.n_runs, rel=1e-9)
