...
def test_c1_fork_sim_failure_blocks_c1_execute_and_c2() -> None:
    ...
    router.strategies['aggressor'].execute_contract_strike = AsyncMock()
    router.strategies['surgeon'].decide_contract_action = Mock()
    router.strategies['surgeon'].execute_contract_decision = AsyncMock()
    router._run_fork_simulation = Mock(return_value={
        'status': 'FAIL',
        'reason': 'slippage_delta_exceeded',
    })

    result = asyncio.run(router.process_discovery_pipeline(route=[{'hop': 1}], raw_spread=0.2))

    router.strategies['aggressor'].execute_contract_strike.assert_not_called()
    router.strategies['surgeon'].decide_contract_action.assert_not_called()
    router.strategies['surgeon'].execute_contract_decision.assert_not_called()
    assert result['c1']['execution']['blocked'] is True
    assert result['c1']['execution']['executed'] is False
    assert result['c2']['plan'] is None
    assert result['pipeline_gate']['blocked_by'] == 'C1_FORK_SIM_FAILED'


def test_c2_fork_sim_failure_blocks_c2_execute() -> None:
    router = ExecutionRouter()
    router.strategies['aggressor'].prepare_contract_strike = Mock(return_value={
        'action': 'EXECUTE',
        'sentinel_output': {'optimal_input': 1000.0, 'profit': 1.0},
    ...
    router.strategies['surgeon'].decide_contract_action = Mock(return_value={
        'action': 'EXECUTE',
        'sentinel_output': {'net_profit_usd': 5.0},
    })
    router.strategies['surgeon'].execute_contract_decision = AsyncMock()

    def fake_fork(leg, _plan, _route):
        if leg == 'C1':
            return {'status': 'PASS', 'leg': leg}
        return {'status': 'FAIL', 'leg': leg, 'reason': 'fork_revert'}
