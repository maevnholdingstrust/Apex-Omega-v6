...
def test_flash_size_candidates_use_exact_ten_percent_weakest_tvl():
    sizes = _flash_size_candidates_usd(
        ...
        max_flash_tvl_fraction=0.10,
        scan_fractions=[0.001, 0.01, 0.10, 0.20],
    )

    assert sizes == [10_000.0]
...
def test_flash_loan_fee_provider_defaults_are_decimal_rates(monkeypatch):
    ...
    assert _resolve_flash_loan_fee_rate("aave_v3") == 0.0005
