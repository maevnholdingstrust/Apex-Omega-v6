...
def _env_optional_int(name: str, default: int | None = None) -> int | None:
    value = os.getenv(name)
    if value in (None, "", "0", "none", "None", "null", "NULL"):
        return default
    return int(value)
...
async def _run_once(cycle_no: int) -> dict[str, Any]:
    ...
    max_scans = _env_optional_int("AUTONOMOUS_SCAN_ROUNDS", 5)
    trade_size_usd = _env_float("AUTONOMOUS_TRADE_SIZE_USD", 10_000.0)
    ...
    payload_routes = _payload_ready_routes(profitable, validation_cfg)
    ...
    return {
        ...
        "payload_ready_count": len(payload_routes),
